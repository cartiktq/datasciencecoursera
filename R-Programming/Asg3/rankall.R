...
rankhospital <- function(stateCode, condition="heart attack", rank="best"){
  ...
  validHospitals <- allHospitals[counts != "Not Available",,drop=FALSE]
...
rankall <- function(condition, rank="best"){
  ...
  for(state in stateCodes){
    hospital <- rankhospital(state, condition, rank)
    hospitals <- rbind(hospitals, c(hospital, state))
  }
  colnames(hospitals) <- c("hospital","state")
  rownames(hospitals) <- stateCodes
  hospitals.df <- data.frame(hospitals)
  hospitals.df
}